...
function fileNaming(names) {
    ...
    while (i<len){
        let j=i+1,
            k=1
        //search and rename all file with same name as names[i]    
        while (j<len){
            if (names[j]==names[i]){
                let t = 0
                //checking if name is not already taken before
                //if so, the index k of the file must increase
                while (t<j){
                    if(names[j]+"("+k+")"==names[t])
                        k++
                    t++
                }
                names[j] = names[j] +"("+k+")"
